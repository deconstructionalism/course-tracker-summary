...
class CSVData {
  ...
  filter (axis = 0, filterFunc, range = null) {
    range = range || [0, this.shape[axis]]
    ...
    process.assert(typeof filterFunc === 'function',
      'filterFunc must be a function')

    const values = axis === 0
      ? this.getRows(...range)
      : this.getColumns(...range)

    return values.filter((element, index, array) => filterFunc(element, index, array))
  }
