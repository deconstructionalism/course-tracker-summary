...
class SheetsClient extends GoogleAuthClient {
  ...
  populateSheetCollectionData (sheetCollectionId) {
    return new Promise((resolve, reject) => {
      const promises = this.sheetCollections[sheetCollectionId].sheets.map(sheet => {
        const { title, rowCount, columnCount } = sheet
        const maxCol = numericToA1(columnCount - 1)
        const range = `${title}!A1:${maxCol}${rowCount}`
        return this.getSheetValues({ spreadsheetId: sheetCollectionId, range })
      })
      Promise.all(promises)
        .then(resArray => this._setSheetCollectionValues(resArray, sheetCollectionId))
        .then(() => resolve(this.sheetCollections[sheetCollectionId]))
        .catch(reject)
    })
  }
    ...
    return new Promise((resolve, reject) => {
      this._api.spreadsheets.get(options, (err, res) => {
        if (err) {
          ...
          resolve(this._addSheetCollection(res))
        }
      })
    })
  }
  ...
  _addSheetCollection (res) {
    ...
    this._sheetCollections[spreadsheetId] = new SheetCollection(spreadsheetId, title, sheets)
  ...
  _setSheetCollectionValues (resArray, sheetCollectionId) {
    resArray.forEach(sheetData => {
      ...
      const sheet = this.sheetCollections[sheetCollectionId].sheets.find(sheet => sheet.title === title)
