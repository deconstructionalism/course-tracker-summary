...
class GmailClient extends GoogleAuthClient {
    ...
    return new Promise((resolve, reject) => {
      if (mailObj.status !== 'QUEUED') {
        ...
        this._api.users.messages.send({
          ...
          err
            ? mailObj.logEvent(MAIL_EVENTS.FAILED_TO_SEND)
            : mailObj.logEvent(MAIL_EVENTS.SENT)
          resolve()
        })
      }
    })
  ...
  // // does not work yest as sentEvent.message.data is not being stored,
  // // as it was causing circular dep issue when trying to save as mailLog
  // checkMailNotBounced (mailObj) {
  ...
  sendAllMail () {
    const promises = this._mailQueue.queue.map((mailObj, index) => {
      return new Promise((resolve, reject) => {
        setTimeout(() => {
